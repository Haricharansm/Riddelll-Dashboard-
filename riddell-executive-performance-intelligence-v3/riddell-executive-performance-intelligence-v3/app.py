...
if page == "Overview":
    ...
    nodes = []
    for source_name, display_name in process_order:
        ...
        nodes.append(
            ...
            <div class="process-node">
                <div class="process-name">{display_name}</div>
                <div class="process-score">{row["score"]:.0f}%</div>
                <div class="process-status {tone}">{status_dot(status)} {label}</div>
                <div class="process-health">Process health</div>
            </div>
            """
        )

    st.markdown('<div class="process-strip">' + "".join(nodes) + "</div>", unsafe_allow_html=True)
