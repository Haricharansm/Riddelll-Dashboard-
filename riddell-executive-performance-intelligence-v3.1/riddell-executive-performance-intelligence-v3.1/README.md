...
# Riddell Executive Performance Intelligence — v3.1
...
## v3 Executive UX Refinements

This package incorporates the design refinements from the live Streamlit review:

- Compact Riddell application header that clears Streamlit's hosted toolbar
- Smaller logo and tighter brand hierarchy
- App-style navigation without numbered prototype labels
- Executive persona and reporting-period controls
- Three compact enterprise pulse cards
- AI Executive Brief as the primary interpretation layer
- Connected six-process health strip
- "Process Health" wording instead of "Composite POC Score"
- Executive exception cards
- Renamed experiences:
  - Overview
  - KPI Stories
  ...
  - Ask Riddell
  - KPI Governance
- Tighter vertical spacing to keep the core leadership story above the fold
- Corporate red kept distinct from semantic KPI status colors

### Recommended customer demo flow

1. **Overview** — establish the enterprise pulse and AI executive brief.
2. **KPI Stories** — show how OTIF becomes a traceable business explanation.
3. **Business Connections** — connect procurement → production → fulfillment → financial exposure.
4. **Ask Riddell** — demonstrate conversational investigation.
5. **KPI Governance** — finish on trust, definitions, source lineage, and data quality.

> All data in this prototype remains illustrative until the approved Riddell KPI list and source mappings are loaded.


---

## v3.1 Streamlit Rendering Fix

The Business Process Health section now uses native Streamlit columns and renders each process card independently.

This fixes an issue seen on hosted Streamlit where a concatenated multi-line HTML block could render the first process card correctly but display the remaining HTML tags as literal text.
